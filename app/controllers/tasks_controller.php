    ...
    function upload() {
        ...
        if (!empty($this->data)) {
            ...
            while (($data = fgetcsv($fp, 10000, ","))) {
                ...
                $col = 0;
                $this->data["Task"]["id"] = trim($data[$col]);
                $col++;
                $this->data["Task"]["sprint_id"] = $this->Sprint->getSprintId($sprints, trim($data[$col]));
                $col++;
                $this->data["Task"]["story_id"] = intval($data[$col]);
                $col++;
                $null1 = $data[$col];
                $col++; // story name
                $this->data["Task"]["name"] = trim($data[$col]);
                $col++;
                $this->data["Task"]["description"] = trim($data[$col]);
                $col++;
                $this->data["Task"]["estimate_hours"] = intval($data[$col]);
                $col++;
                $this->data["Task"]["user_id"] = $this->User->getUserId($users, trim($data[$col]));
                $col++;
                $this->data["Task"]["resolution_id"] = $this->Resolution->getResolutionId($resolutions, trim($data[$col]));
                $col++;
