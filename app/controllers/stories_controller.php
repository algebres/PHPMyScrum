    ...
    function upload() {
        ...
        if (!empty($this->data)) {
            ...
            while (($data = fgetcsv($fp, 10000, ","))) {
                ...
                $col = 0;
                $this->data["Story"]["id"] = trim($data[$col]);
                $col++;
                $this->data["Story"]["name"] = trim($data[$col]);
                $col++;
                $this->data["Story"]["description"] = trim($data[$col]);
                $col++;
                $this->data["Story"]["storypoints"] = intval($data[$col]);
                $col++;
                $null1 = $data[$col];
                $col++;
                $null2 = $data[$col];
                $col++;
                $this->data["Story"]["businessvalue"] = intval($data[$col]);
                $col++;
                $sprint = $data[$col];
                $col++;
                $priority = $data[$col];
